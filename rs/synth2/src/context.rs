...
impl<const N: usize> AudioContext<N> {
    ...
    pub fn add_mono_node<T>(&mut self, node: T) -> Result<NodeIndex, GlicolSynthError>
    where T: Node<N> + Send + 'static,
    {
        if self.config.channels != 1 {
            return Err(GlicolSynthError::ChannelMismatchingError)
        }
    ...
    pub fn add_stereo_node<T>(&mut self, node: T) -> Result<NodeIndex, GlicolSynthError>
    where T: Node<N> + Send + 'static,
    {
    ...
    pub fn add_multi_chan_node<T>(&mut self, chan: usize, node: T)
    -> Result<NodeIndex, GlicolSynthError>
    where T: Node<N> + Send + 'static,
    {
        if self.config.channels != chan {
            return Err(GlicolSynthError::ChannelMismatchingError)
        }
        let node_index = self.graph.add_node( // channel?
